    ...
    <div className="relative mx-auto max-w-2xl pb-20">
      ...
      {path.units.map((unit) => (
        <section key={unit.id} className="relative mb-10">
          <div className="sticky top-16 z-10 mx-auto mb-8 rounded-2xl border-b-4 border-[#46a302] bg-[#58cc02] px-6 py-4 text-white shadow-sm">
            <div className="text-xs font-black uppercase">Unit {unit.number}</div>
            <h2 className="text-2xl font-black">{unit.title}</h2>
            <p className="text-sm font-bold text-green-50">{unit.description}</p>
          </div>
