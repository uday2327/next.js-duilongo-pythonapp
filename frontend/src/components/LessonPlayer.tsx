  ...
  if (hearts <= 0 && !complete) {
    return (
      <div className="grid min-h-screen place-items-center bg-[var(--background)] p-6 text-center">
        <div className="max-w-md rounded-3xl border-2 border-slate-100 p-8 shadow-xl bg-card">
          <div className="text-6xl">Heart</div>
          <h1 className="mt-4 text-3xl font-black text-[var(--foreground)]">Out of hearts!</h1>
          <p className="mt-2 font-semibold text-muted">Practice to earn hearts back or refill them in the shop.</p>
          <div className="mt-6 flex gap-3">
            ...
            <Link href="/shop" className="flex-1 rounded-2xl border-b-4 border-accent bg-accent px-5 py-3 font-black text-white">Refill</Link>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="grid min-h-screen place-items-center bg-[var(--background)] p-6 text-center">
        <div className="max-w-xl">
          <div className="mx-auto grid h-32 w-32 place-items-center rounded-full bg-accent text-5xl font-black text-white shadow-xl">Duo</div>
          <h1 className="mt-6 text-5xl font-black text-[var(--foreground)]">Lesson complete!</h1>
          <p className="mt-3 text-xl font-bold text-muted">Score {complete.score}%</p>
          <div className="mt-8 grid grid-cols-2 gap-4">
            <div className="rounded-2xl border-2 border-amber-200 bg-card p-5 font-black text-amber-600">+{complete.earned_xp} XP</div>
            <div className="rounded-2xl border-2 border-rose-200 bg-card p-5 font-black text-rose-500">{hearts} hearts left</div>
          </div>
          <button onClick={() => router.push("/learn")} className="mt-8 w-full rounded-2xl border-b-4 border-accent bg-accent px-8 py-4 font-black uppercase text-white">Back to path</button>
        </div>
      </div>
    );
  }
    ...
    <main className="mx-auto flex min-h-screen max-w-4xl flex-col px-4 pb-32 pt-6">
      <div className="flex items-center gap-4">
        <button onClick={() => confirm("Leave lesson? Your progress in this lesson will not be saved.") && router.push("/learn")} aria-label="Leave lesson" className="text-slate-400"><X size={32} /></button>
          <div className="h-5 flex-1 rounded-full bg-slate-100"><div className="h-full rounded-full bg-accent transition-all" style={{ width: `${progress}%` }} /></div>
        <Hearts count={hearts} />
      </div>
      <section className="flex flex-1 flex-col justify-center py-14">
        <p className="mb-2 font-black uppercase text-accent">{exercise.instruction}</p>
      ...
      {!submitted && exercise.type !== "match_pairs" && (
        <button onClick={() => submit()} className="ml-auto rounded-2xl border-b-4 border-accent bg-accent px-10 py-4 font-black uppercase text-white disabled:border-slate-300 disabled:bg-slate-200" disabled={!answer || (Array.isArray(answer) && answer.length === 0)}>
          Check
        </button>
      )}
