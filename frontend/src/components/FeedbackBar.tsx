...
export function FeedbackBar({ correct, answer, onContinue }: { correct: boolean; answer: string; onContinue: () => void }) {
  return (
    <div className={`fixed inset-x-0 bottom-0 z-30 border-t-2 px-4 py-5 ${correct ? "border-accent bg-accent-10" : "border-red-200 bg-red-100"}`}>
      <div className="mx-auto flex max-w-4xl flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <button onClick={onContinue} className={`w-full rounded-2xl border-b-4 px-8 py-3 font-black uppercase text-white sm:w-auto ${correct ? "border-accent bg-accent" : "border-red-700 bg-red-500"}`}>
          Continue
        </button>
      </div>
    </div>
