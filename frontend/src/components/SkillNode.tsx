  ...
  const content = (
    <div className={cx("group relative flex w-56 flex-col items-center gap-2", offset === "left" && "mr-28", offset === "right" && "ml-28")}>
      <ProgressRing progress={skill.progress}>
        <div className={cx("grid h-16 w-16 place-items-center rounded-full border-b-8 text-white shadow-lg transition group-hover:translate-y-1", locked ? "border-slate-400 bg-slate-300" : completed ? "border-amber-500 bg-amber-400" : "border-accent bg-accent")}>
      ...
      {!locked && (
        <div className="absolute -right-10 top-8 rounded-full bg-white p-2 text-accent shadow">
