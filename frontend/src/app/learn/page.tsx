        ...
        <aside className="space-y-4">
          <div className="rounded-2xl border-2 border-slate-100 bg-white p-4">
            ...
            <div className="mt-3 h-4 rounded-full bg-slate-100">
              <div className="h-full rounded-full bg-[#58cc02]" style={{ width: `${stats ? Math.min(100, (stats.today_goal_progress / stats.daily_goal) * 100) : 0}%` }} />
            </div>
          ...
          <div className="rounded-2xl border-2 border-slate-100 bg-white p-4">
            ...
            {leaders.map((entry) => (
              <div key={entry.id} className={`flex justify-between rounded-xl px-3 py-2 font-bold ${entry.is_current_user ? "bg-[#ddf4ff] text-[#1cb0f6]" : "text-slate-600"}`}>
