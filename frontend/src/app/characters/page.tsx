    ...
    <AppShell>
      <main className="mx-auto max-w-4xl px-4 py-8">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          {cards.map((card) => <button key={card.sound} onClick={() => speechSynthesis?.speak(new SpeechSynthesisUtterance(card.word))} className="rounded-2xl border-2 border-b-4 border-slate-100 bg-card p-6 text-left hover-dark"><Volume2 className="text-accent" /><div className="mt-4 text-4xl font-black text-[var(--foreground)]">{card.sound}</div><div className="mt-1 text-xl font-black text-accent">{card.word}</div><p className="mt-2 font-bold text-muted">{card.hint}</p></button>)}
        </div>
      </main>
    </AppShell>
  );
